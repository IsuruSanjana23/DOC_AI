...
class TestPromptBuilderBuild:
    ...
    def test_contains_system_instructions(
    ...
    def test_multiple_chunks_are_numbered(
        self, builder: PromptBuilder
    ) -> None:
        results = [
    ...
    def test_chunks_separated_by_blank_line(
        ...
        # Each chunk is separated by a blank line in context_blocks
...
class TestEdgeCases:
    ...
    def test_empty_context_omits_context_header(
        self, builder: PromptBuilder
    ) -> None:
        prompt = builder.build("Hello?", [])
    ...
    def test_empty_chunk_text_raises_error(
        ...
        with pytest.raises(
            PromptBuilderError, match="empty text"
        ):
