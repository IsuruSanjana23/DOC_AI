...
# ── Default Templates ──────────────────────────────────────────────────────
# Each template uses str.format()-style placeholders. Override any of them
# via constructor injection to change prompt format without subclassing.
...
class Prompt:
    ...
    :param context: Formatted retrieved-chunk context (empty if no
        results were provided).
    :param question: The user question framed inside the prompt template.
    :param text: Fully assembled prompt string (system + context + question).
    :param token_count: Estimated token count. Currently uses character
        count as a proxy. Replace with a real tokenizer when token-level
        control is needed.
...
class PromptBuilder:
    ...
    assembles them into an immutable :class:`Prompt` dataclass.
    ...
    Usage::
        ...
        # prompt.text       → fully assembled prompt string
    ...
    def build(self, query: str, results: list[SearchResult]) -> Prompt:
        ...
        :raises PromptBuilderError: If any chunk has empty text.
        """
        ...
        logger.info(
            "Prompt built — system=%d context=%d question=%d "
            "total=%d tokens=%d chunks=%d",
            len(system),
            len(context),
            len(question),
            len(text),
    ...
    def _build_system_prompt(self) -> str:
        """Return the formatted system instructions.

        This method exists as a separate helper so that subclasses or
        future versions can inject dynamic content (date, user name,
        tone override) without changing ``build()``.
        """
    ...
    def _build_context(self, results: list[SearchResult]) -> str:
        ...
        :param results: Retrieved search results. May be empty.
    ...
    def _format_chunk(self, source_number: int, result: SearchResult) -> str:
        """Format a single search result into a chunk block.
    ...
    def _build_question(self, query: str) -> str:
        """Frame the validated query inside the question template.
    ...
    @staticmethod
    def _assemble(system: str, context: str, question: str) -> str:
        """Concatenate the three prompt sections into one string.

        Sections are separated by blank lines. If there is no context,
        it is omitted entirely to avoid a stray ``Context:\n\n`` block.

        :param system: Formatted system instructions.
        :param context: Formatted context block (may be empty).
        :param question: Formatted question block.
    ...
    def _estimate_tokens(text: str) -> int:
        ...
        when token-level control (truncation, budgeting) is needed.
